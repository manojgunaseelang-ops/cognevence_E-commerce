...
export default function Acessories() {
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      try {
        ...
        setDetailError('Unable to load accessories at the moment. Please try again later.');
      }
    };
    fetchAccessories();
  }, []);
  ...
  const handleFetchDetail = async (id) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        setDetailError(data.error || 'Unable to load accessory details');
        return;
      }
      ...
      setDetailError('Unable to fetch accessory details.');
    }
  };
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">Discover stylish accessories for every need.</p>
          </div>
          {detailError && (
        ...
        <div className="mb-6 grid gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-4">
          <input
            ...
            className="rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none ring-0 focus:border-blue-500"
          />
          <select
            name="gender"
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          {filteredAccessories.map((accessory) => (
            <div key={accessory._id || accessory.id} className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
              <img
                src={accessory.images?.[0]?.image || DEFAULT_IMAGE_PLACEHOLDER}
                alt={accessory.name}
                className="h-56 w-full object-contain p-3"
              />
              <div className="flex flex-1 flex-col p-5">
                <h3 className="text-lg font-semibold text-gray-900">{accessory.name}</h3>
                <p className="mt-1 text-sm text-gray-600">Brand: {accessory.brand}</p>
                <p className="mt-2 text-xl font-bold text-blue-600">₹{Number(accessory.price || 0).toLocaleString()}</p>
                <p className="mt-1 text-sm text-gray-500">Stock: {getProductStock(accessory)}</p>
                <div className="mt-auto flex gap-2 pt-4">
                  <button
                    type="button"
                    onClick={() => handleViewProduct(accessory)}
                  ...
                  <button
                    ...
                    disabled={getProductStock(accessory) <= 0}
          ...
          <div className="product-view-overlay" onClick={() => setSelectedProduct(null)}>
            <div className="product-view-modal" onClick={(e) => e.stopPropagation()}>
              <div className="grid gap-6 lg:grid-cols-[1.05fr,0.95fr]">
                ...
                <p><span className="font-semibold">Gender:</span> {selectedProduct.gender || 'N/A'}</p>
              </div>
