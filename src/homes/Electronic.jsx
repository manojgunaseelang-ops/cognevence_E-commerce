...
const getProductStock = (product) => {
  if (product?.sizes?.length) {
    return product.sizes.reduce((sum, sizeObject) => {
      if (typeof sizeObject === 'object' && sizeObject !== null) {
        return sum + (Number(sizeObject.stock) || 0);
      }
      return sum;
    }, 0);
  }
...
export default function Electronic() {
  ...
  useEffect(() => {
    const fetchElectronics = async () => {
      try {
        const response = await fetch(`${API_BASE}/electronic`);
        const data = await response.json();
        setElectronics(data.data || []);
      } catch (error) {
        console.error('Error fetching electronic data:', error);
        setDetailError('Unable to load electronics at the moment. Please try again later.');
      }
    };

    withLoading(fetchElectronics).catch((error) => {
      console.error('Error during electronic page load:', error);
    });
  }, [withLoading]);
  ...
  const handleFetchDetail = async (id) => {
    ...
    try {
      await withLoading(async () => {
        const response = await fetch(`${API_BASE}/electronic/${id}`);
        const data = await response.json();
        if (!response.ok) {
      ...
      console.error('Error loading electronic detail:', error);
      setDetailError('Unable to fetch electronic product details.');
    }
  };
  ...
  const getAvailableStock = (product) => {
    if (!product) return 0;
    if (product.sizes?.length) {
      const sizeObject = product.sizes.find((size) => size === selectedSize || size.size === selectedSize);
      return sizeObject?.stock ?? getProductStock(product);
    }
  ...
  const handleAddToCart = (product) => {
    ...
    if (product.sizes?.length > 0) {
      ...
      setModalError('Please select a size before adding to cart.');
      return;
    }
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">Browse the latest gadgets and smart essentials.</p>
          </div>
          {detailError && (
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          {filteredProducts.map((electronic) => (
            <div key={electronic._id || electronic.id} className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
              <img
                src={electronic.images?.[0]?.image || DEFAULT_IMAGE_PLACEHOLDER}
                alt={electronic.name}
                className="h-56 w-full object-contain p-3"
              />
              <div className="flex flex-1 flex-col p-5">
                <h3 className="text-lg font-semibold text-gray-900">{electronic.name}</h3>
                <p className="mt-1 text-sm text-gray-600">Brand: {electronic.brand}</p>
                <p className="mt-1 text-sm text-gray-600">Category: {electronic.category}</p>
                <p className="mt-2 text-xl font-bold text-blue-600">₹{Number(electronic.price || 0).toLocaleString()}</p>
                <p className="mt-1 text-sm text-gray-500">Stock: {getProductStock(electronic)}</p>
                <div className="mt-auto flex gap-2 pt-4">
                  <button type="button" onClick={() => handleViewProduct(electronic)} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                    View
                  </button>
                  <button
                    type="button"
                    onClick={() => handleAddToCart(electronic)}
                    disabled={getProductStock(electronic) <= 0}
                    className="flex-1 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-gray-400"
                  >
                    Add to Cart
                  </button>
                </div>
            ...
            <div className="product-view-modal" onClick={(e) => e.stopPropagation()}>
              <div className="grid gap-6 lg:grid-cols-[1.05fr,0.95fr]">
                <div className="flex items-center justify-center rounded-2xl bg-gray-50 p-3 sm:p-4">
                  <img
                    src={selectedProduct.images?.[0]?.image || DEFAULT_IMAGE_PLACEHOLDER}
                ...
                <div className="mt-5">
                  ...
                  >
                    ...
                    {selectedProduct.sizes.map((sizeObject) => (
                      <option key={sizeObject.size || sizeObject} value={sizeObject.size || sizeObject}>
                        {sizeObject.size || sizeObject} ({sizeObject.stock ?? '0'} available)
                      </option>
                    ))}
                  </select>
                </div>
              ) : (
                <p className="mt-5 text-sm text-gray-600">No size selection needed for this product.</p>
              )}
